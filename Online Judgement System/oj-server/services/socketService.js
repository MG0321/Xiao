...
//传入了io才能对socket进行连接和对消息进行收发
module.exports = function (io) {
    /**
     *io.on('connection',(socket)=>{
  *  console.log("socketService: "+socket);
  *  //handshake是为了接收对方发来的消息
  ...
  *});
    ...
    //socketID是连接在socket端的用户的id; seesionID是问题的id
    ...
    if (sessionId in collaborations) {
        ...
        for (let i = 0; i < changeEvents.length; i++) {
            socket.emit(changeEvents[i][0], changeEvents[i][1]);//[0]→'change', [1]→delta
        }
    }
})
